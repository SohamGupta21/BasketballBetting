...
def sim_season(team1, team2, games=82):
    ...
    for team in [team1, team2]:
        ...
        for p in team.players:
            p.fga = p.fg2a + p.fg3a
            p.fgm = p.fg2m + p.fg3m
            ppg = '%.1f' % (p.points/games)
            fg_string = '%.1f' % (100*p.fgm/p.fga)
            try:
                fg3_string = '%.1f' % (100*p.fg3m/p.fg3a)
            except:
                fg3_string = '0.0'
            fgag = '%.1f' % (p.fga/games)
            apg = '%2.1f' % (p.assists/games)
            topg = '%.1f' % (p.turnovers/games)
            spg = '%.1f' % (p.steals/games)
            bpg = '%.1f' % (p.blocks/games)
            rpg = '%.1f' % (p.rebounds/games)
            orpg = '%.1f' % (p.off_rebounds/games)
    ...
    print('')
    t1ppg = '%.1f' % (team1.season_points/games)
    t2ppg = '%.1f' % (team2.season_points/games)
...
def sim_game(team1, team2, pbp=False):
    ...
    for team in [team1, team2]:
        ...
        for p in team.players:
            ...
            time_played_str = str(p.time_played).split('.')[0]  # Remove microseconds
            print('|{0:<{n}}|{1:<{p}}|{2:<{f}}|{3:<{t}}|{4:<{t}}|{5:<{t}}|{6:<{t}}|{7:<{t}}|{8:<{t}}|{9:<{f}}|{10:<{f}}|'.format(p.name,
            p.points, fg_string, fg3_string, p.assists, p.turnovers, p.steals, p.blocks, p.rebounds, p.off_rebounds, time_played_str, n=20, p=3, f=6, t=5))
